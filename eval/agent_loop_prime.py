...
def build_reward_contract() -> dict[str, Any]:
    return {
        ...
        "environment_id": "inference/extension3-agent-loop",
        "track_mapping": {
...
def build_prime_training_config() -> dict[str, Any]:
    return {
        ...
        "model": {
            "candidate_base": "Qwen/Qwen3.5-2B",
            "adapter": "lora",
            "selection_reason": "Listed by Prime Hosted Training, available at smoke time, and cheap enough for the first pilot.",
        },
        "hardware": {
        ...
        "environment": {
            "id": "inference/extension3-agent-loop",
...
def _training_toml(config: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f'model = "{config["model"]["candidate_base"]}"',
            'loss = "rl"',
            "max_steps = 50",
            "batch_size = 64",
            ...
            f'keep_cloud = {config["checkpointing"]["keep_cloud_checkpoints"]}',
            "",
            "[adapters]",
            ...
            'args = { split = "train", max_examples = 12, include_hard = true }',
            "",
            "[[eval.env]]",
            f'id = "{config["environment"]["id"]}"',
            'args = { split = "eval", max_examples = 6, include_hard = true }',
            "num_examples = 6",
            "rollouts_per_example = 1",
            "",
        ]
...
def _runbook_markdown(packet: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Full checkpoints every `{training['checkpointing']['checkpoint_interval']}` steps with `keep_cloud = {training['checkpointing']['keep_cloud_checkpoints']}`.",
