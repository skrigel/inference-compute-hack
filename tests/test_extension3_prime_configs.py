...
class Extension3PrimeConfigTests(unittest.TestCase):
    def test_full_budget_config_uses_large_model_and_large_retrieval_data(self):
        ...
        self.assertEqual(config["eval"]["interval"], 25)
        self.assertEqual(config["eval"]["num_examples"], 72)
        self.assertEqual(config["eval"]["rollouts_per_example"], 1)
        self.assertFalse(config["eval"]["skip_first_step"])
